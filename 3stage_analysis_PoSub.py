...
def tuning_curves(NWB):
    ...
    #Remove cells firing above 20Hz
    max_firing = tuning_curves.max(dim="head_direction")
    valid_units = max_firing.where(max_firing <= 20, drop=True).unit
    
    tuning_curves = tuning_curves.sel(unit=valid_units)
      
    #Check if any cells left
    if tuning_curves.sizes.get("unit", 0) == 0:
        return None      
...
def avg_firing(NWB, stage):
    ...
    #Remove cells firing above 20Hz
    max_firing = tuning_curves.max(dim="head_direction")
    valid_units = max_firing.where(max_firing <= 20, drop=True).unit.values
...
def continuous_stability(NWB, stage, plot_comparison=False):
    ...
    #Remove cells firing above 20Hz
    tuning_full = tuning_full.sel(unit = HD_cells)
    max_full = tuning_full.max(dim ="head_direction")
    valid_units = max_full.where(max_full <= 20, drop = True).unit.values
...
def interleaved_stability(NWB, stage, plot_stability=False):
    ...
    #Check if any cells left
    if tuning_full.sizes.get("unit", 0) == 0:
        return None      
...
#plot tuning curves
'''
md_tuning = [tc for nwb in md_nwb if (tc := tuning_curves(nwb)) is not None]
#plot_curves(md_tuning, "Metestrus/Diestrus", "#56B4E9", polar = True)

e_tuning = [tc for nwb in e_nwb if (tc := tuning_curves(nwb)) is not None]
#plot_curves(e_tuning, "Estrus", "#54BBAB", True)

p_tuning = [tc for nwb in p_nwb if (tc := tuning_curves(nwb)) is not None]
#plot_curves(p_tuning, "Proestrus", "#CC79A7", True)
...
violin_plot(
    ...
    title = "PoSub HD Cell Coding Property: Maximum Firing Rate During Exploration", 
    ref_line = None,
    )
...
violin_plot(
    ...
    title = "PoSub HD Cell Coding Property: Tuning Width (Full Width at Half Maximum)", 
    ref_line = None)

#plot avg firing
md_avg_firing = pd.concat([af for nwb in md_nwb if (af := avg_firing(nwb, "Metestrus/Diestrus")) is not None])
e_avg_firing = pd.concat([af for nwb in e_nwb if (af := avg_firing(nwb, "Estrus")) is not None])
p_avg_firing = pd.concat([af for nwb in p_nwb if (af := avg_firing(nwb, "Proestrus")) is not None])
...
violin_plot(
    ...
    title = "PoSub HD Cell Coding Property: Average Firing During Sleep", 
    ref_line = None)
  
#plot continuous stability (can plot comparision plots if add True)

md_cont_stab = pd.concat([cs for nwb in md_nwb if (cs := continuous_stability(nwb, "Metestrus/Diestrus")) is not None])
e_cont_stab = pd.concat([cs for nwb in e_nwb if (cs := continuous_stability(nwb, "Estrus")) is not None])
p_cont_stab = pd.concat([cs for nwb in p_nwb if (cs := continuous_stability(nwb, "Proestrus")) is not None])
...
violin_plot(
    ...
    title = "PoSub HD Cell Coding Property: Continuous Stability", 
    ref_line = 0.8, 
    run_stats=False)
...
'''
md_inter_stab = pd.concat([is_ for nwb in md_nwb if (is_ := interleaved_stability(nwb, "Metestrus/Diestrus")) is not None])
e_inter_stab = pd.concat([is_ for nwb in e_nwb if (is_ := interleaved_stability(nwb, "Estrus")) is not None])
p_inter_stab = pd.concat([is_ for nwb in p_nwb if (is_ := interleaved_stability(nwb, "Proestrus")) is not None])
...
violin_plot(
    ...
    y = "tuning_corr",
    ylabel = "Pearson Correlation (r)", 
    title = "PoSub HD Cell Coding Property: Interlevered Stability", 
    ref_line = 0.8, 
    run_stats=False)
